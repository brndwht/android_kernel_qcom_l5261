...
#if defined(CONFIG_PROJECT_L5261) || defined(CONFIG_PROJECT_L5421)
...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	...
	//yangliang add for enable pa mode-2;20150901 changed enable/disable sequences based on aw8155(or other speaker pa in the future) datasheet.
	if(enable) {
		/*              _____________
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (pdata->spk_ext_pa_gpio < 0) {
		...
		gpio_direction_output(pdata->spk_ext_pa_gpio, 0);//LINE<20150805><init ext pa gpio>wangyanhui 
	}
