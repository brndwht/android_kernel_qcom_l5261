...
struct led_gpio_flash_data {
	...
	int flash_front_en;
	int flash_front_now;
	struct msm_flash_ctrl_seq ctrl_seq_front[2];	
};
				    ...
				    enum led_brightness value)
{
	...
	//pr_info(  " led_gpio_brightness_set  msm_sensor_is_front_camera = %d " ,msm_sensor_is_front_camera());
if(msm_sensor_is_front_camera() == 0)
{
...
else
{
	if (brightness > LED_HALF) {
		flash_en =
			flash_led->ctrl_seq_front[FLASH_EN].flash_on_val;
		flash_now =
			flash_led->ctrl_seq_front[FLASH_NOW].flash_on_val;
	} else if (brightness > LED_OFF) {
		flash_en =
			flash_led->ctrl_seq_front[FLASH_EN].torch_on_val;
		flash_now =
			flash_led->ctrl_seq_front[FLASH_NOW].torch_on_val;
	} else {
		flash_en = 0;
		flash_now = 0;
	}
	...
	rc = gpio_direction_output(flash_led->flash_front_en, flash_en);
	if (rc) {
		pr_err("%s: Failed to set gpio %d\n", __func__,
		       flash_led->flash_front_en);
		goto err;
	}
	rc = gpio_direction_output(flash_led->flash_front_now, flash_now);
	if (rc) {
		pr_err("%s: Failed to set gpio %d\n", __func__,
		       flash_led->flash_front_now);
		goto err;
	}	
}
...
int led_gpio_flash_probe(struct platform_device *pdev)
{
...
#if 1
	flash_led->flash_front_en = of_get_named_gpio(node, "qcom,flash-front-en", 0);
	if (flash_led->flash_front_en < 0) {
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed. rc =  %d\n",
			"flash-en", node->full_name, flash_led->flash_front_en);
		goto error;
	} else {
		rc = gpio_request(flash_led->flash_front_en, "FLASH_FRONT_EN");
		if (rc) {
			dev_err(&pdev->dev,
				"%s: Failed to request gpio front %d,rc = %d\n",
				__func__, flash_led->flash_front_en, rc);

			goto error;
		}
	}

	flash_led->flash_front_now = of_get_named_gpio(node, "qcom,flash-front-now", 0);
	if (flash_led->flash_front_now < 0) {
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed.  front rc =  %d\n",
			"flash-now", node->full_name, flash_led->flash_now);
		goto error;
	} else {
		rc = gpio_request(flash_led->flash_front_now, "FLASH_FRONT_NOW");
		if (rc) {
			dev_err(&pdev->dev,
				"%s: Failed to request gpio front %d,rc = %d\n",
				__func__, flash_led->flash_front_now, rc);
			goto error;
		}
	}
	rc = of_property_read_u32_array(node, "qcom,flash-seq-val-front",
		array_flash_seq, 2);

	if (rc < 0) {
		pr_err("%s get flash front op seq failed front %d\n",
			__func__, __LINE__);
		goto error;
	}

	rc = of_property_read_u32_array(node, "qcom,torch-seq-val-front",
		array_torch_seq, 2);

	if (rc < 0) {
		pr_err("%s get torch op seq failed  front  %d\n",
			__func__, __LINE__);
		goto error;
	}
	...
	for (i = 0; i < 2; i++) {
		rc = of_property_read_string_index(node,
			"qcom,op-seq-front", i,
			&seq_name);
		CDBG("%s front seq_name[%d] = %s\n", __func__, i,
			seq_name);
		if (rc < 0)
			dev_err(&pdev->dev, "%s failed  front %d\n",
				__func__, __LINE__);

		if (!strcmp(seq_name, "flash_front_en")) {
			flash_led->ctrl_seq_front[FLASH_EN].seq_type =
				FLASH_EN;
			CDBG("%s:%d  front seq_type[%d] %d\n", __func__, __LINE__,
				i, flash_led->ctrl_seq_front[FLASH_EN].seq_type);
			if (array_flash_seq[i] == 0)
				flash_led->ctrl_seq_front[FLASH_EN].flash_on_val =
					GPIO_OUT_LOW;
			else
			...
			if (array_torch_seq[i] == 0)
				...
				flash_led->ctrl_seq_front[FLASH_EN].torch_on_val =
					GPIO_OUT_HIGH;
		} else if (!strcmp(seq_name, "flash_front_now")) {
			flash_led->ctrl_seq_front[FLASH_NOW].seq_type =
				FLASH_NOW;
			CDBG("%s:%d front seq_type[%d] %d\n", __func__, __LINE__,
				i, flash_led->ctrl_seq_front[i].seq_type);
			if (array_flash_seq[i] == 0)
				flash_led->ctrl_seq_front[FLASH_NOW].flash_on_val =
					GPIO_OUT_LOW;
			else
