...
struct qpnp_lbc_chip {
	...
	//BEGIN  <DATA20150603> <tinno battery temp manger> lishengli
	...
	//END  <DATA20150603> <tinno battery temp manger> lishengli
...
struct qpnp_lbc_chip *extern_chip;
int g_bat_init_flag = 0;
int usb_plugged_in(void)
{
    return qpnp_lbc_is_usb_chg_plugged_in(extern_chip);
}
#endif
...
#define QPNP_LBC_IBATMAX_MAX	1000	//1440
/*
...
static void qpnp_lbc_set_appropriate_current(struct qpnp_lbc_chip *chip)
{
	...
	if (chip->therm_lvl_sel >= 0 && chip->thermal_mitigation)			//add by lishengli 20150819   limit current by dts setting
		chg_current = min(chg_current,
			chip->thermal_mitigation[chip->therm_lvl_sel]);

	pr_info("setting charger current %d mA ,   lishengli therm_lvl_sel = %d\n", chg_current, chip->therm_lvl_sel);
