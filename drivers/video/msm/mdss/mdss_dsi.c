...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	for (i = DSI_MAX_PM - 1; i >= 0; i--) {
     ...
     //Begin<REQ><20150925><add for L5421 LCD power on>xiongdajun
        ...
        /*if (!gpio_is_valid(ctrl_pdata->power18_gpio)) {
        		pr_debug("%s:%d, power18_gpio line not configured\n",
        			   __func__, __LINE__);
        		goto end;
        	}
		...
		gpio_set_value((ctrl_pdata->power28_gpio), 0);
		gpio_free(ctrl_pdata->power28_gpio);
        gpio_set_value((ctrl_pdata->power18_gpio), 0);
		gpio_free(ctrl_pdata->power18_gpio);*/
       gpio_direction_output((ctrl_pdata->power18_gpio),0);
       gpio_direction_output((ctrl_pdata->power28_gpio),0);			
        #endif
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
    ...
    //Begin<REQ><20150925><add for L5421 LCD power on>xiongdajun
      ...
      /* if (!gpio_is_valid(ctrl_pdata->power18_gpio)) {
       ...
       gpio_set_value((ctrl_pdata->power18_gpio),1);
       mdelay(3);//wangyanhui add for delay 
       gpio_set_value((ctrl_pdata->power28_gpio),1);
       mdelay(50);*/
       gpio_direction_output((ctrl_pdata->power18_gpio),1);
       mdelay(3);//wangyanhui add for delay 
       gpio_direction_output((ctrl_pdata->power28_gpio),1);
       mdelay(50);       
       #endif
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
    ...
    //Begin<REQ><20150925><add for L5421 LCD power on>xiongdajun
     #if defined(CONFIG_PROJECT_L5421)
	ctrl_pdata->power28_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-power28-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->power28_gpio))
		pr_err("%s:%d, reset gpio not specified\n",
						__func__, __LINE__);
	ctrl_pdata->power18_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-power18-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->power18_gpio))
		pr_err("%s:%d, reset gpio not specified\n",
						__func__, __LINE__);
        ...
        if (rc) {
    		pr_err("request power power28_gpio failed, rc=%d\n",
    			rc);
        }
        rc = gpio_request(ctrl_pdata->power18_gpio, "power18_gpio");
        if (rc) {
    		pr_err("request power power18_gpio failed, rc=%d\n",
    			rc);
        }	
    #endif
    //END<REQ><20150925><add for L5421 LCD power on>xiongdajun
