...
int oldsun_status = 0;
int sun_status = 0;
extern int show_sub_sun_panle(void);
#endif
				      ...
				      enum led_brightness value)
{
	...
	if (!IS_CALIB_MODE_BL(mfd) && (!mfd->ext_bl_ctrl || !value ||
        ...
        //Begin<JABALL-1500><20150623>Add sun panel function for L5510;xiongdajun
        #if defined(CONFIG_SUN_PANEL_FUNCTION)
            pr_debug("xiongdajun add %d bl_level:%d\n",__LINE__,bl_lvl);
             if(!show_sub_sun_panle())
                sun_status = 0;             //AIE  OFF
             else if(show_sub_sun_panle())
             {
                if(bl_lvl>254)
                {
                    sun_status = 2;         //AIE  Hight
                }
                else
                    sun_status = 0;             //AIE  OFF
             }
