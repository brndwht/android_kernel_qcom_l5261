...
static int mdss_dsi_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
    ...
    //Begin<REQ><20150925><add for L5421 LCD power on>xiongdajun
       #if 0//defined(CONFIG_PROJECT_L5421)
        rc = gpio_request(ctrl_pdata->power28_gpio, "power28_gpio");
    	if (rc) {
    		pr_err("request power gpio failed, rc=%d\n",
    			rc);
    		goto power28_gpio_err;
    	}
        rc = gpio_request(ctrl_pdata->power18_gpio, "power18_gpio");
    	if (rc) {
    		pr_err("request power gpio failed, rc=%d\n",
    			rc);
    		goto power18_gpio_err;
    	}
        #endif
       //END<REQ><20150925><add for L5421 LCD power on>xiongdajun
	return rc;

//Begin<REQ><20150925><add for L5421 LCD power on>xiongdajun
#if 0//defined(CONFIG_PROJECT_L5421)
power18_gpio_err:
    if (gpio_is_valid(ctrl_pdata->power28_gpio))
		gpio_free(ctrl_pdata->power28_gpio);
power28_gpio_err:
    if (gpio_is_valid(ctrl_pdata->mode_gpio))
...
//Begin<JABALL-1500><20150623>Add sun panel function for L5510;xiongdajun
#if defined(CONFIG_SUN_PANEL_FUNCTION)
extern int oldsun_status;
extern int sun_status;
#endif
//ENE<JABALL-1500><20150623>Add sun panel function for L5510;xiongdajun
							...
							u32 bl_level)
{
	...
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	#endif
	//END<JABALL-1500><20150623>Add sun panel function for L5510;xiongdajun
	if (pdata == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
		return;
	}
		...
		mdss_dsi_panel_bklt_pwm(ctrl_pdata, bl_level);
             ...
             if(sun_status != oldsun_status)
            {
                ...
                if(sun_status==2)
                {
                    ...
                    if (ctrl->aiehi_cmds.cmd_cnt) {
                    mdss_dsi_panel_cmds_send(ctrl, &ctrl->aiehi_cmds);
                    }
                }
                else if(sun_status == 1)
                 {
                    pr_debug("%s: ctrl=%p ndx=%d\n", __func__, ctrl, ctrl->ndx);
                    if (ctrl->aielow_cmds.cmd_cnt) {
                    mdss_dsi_panel_cmds_send(ctrl, &ctrl->aielow_cmds);
                        }
                }
                else if(sun_status == 0)
               {
                pr_debug("%s: ctrl=%p ndx=%d\n", __func__, ctrl, ctrl->ndx);
                if (ctrl->aieoff_cmds.cmd_cnt) {
                mdss_dsi_panel_cmds_send(ctrl, &ctrl->aieoff_cmds);
                    }
                } 
           }
             oldsun_status = sun_status;
           #endif
       //END <REQ><JABALL-1500><20150623>Add sun panel function for L5510;xiongdajn
		break;
	case BL_DCS_CMD:
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	//Begin<JABALL-1500><20150623>Add sun panel function for L5510;xiongdajun
	#if defined (CONFIG_SUN_PANEL_FUNCTION)
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->aiehi_cmds,
	"qcom,mdss-dsi-panel-aiehi-command", "qcom,mdss-dsi-panel-aiehi-command-state");
       mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->aielow_cmds,
	"qcom,mdss-dsi-panel-aielow-command", "qcom,mdss-dsi-panel-aielow-command-state"); 
       mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->aieoff_cmds,
	"qcom,mdss-dsi-panel-aieoff-command", "qcom,mdss-dsi-panel-aieoff-command-state"); 
	#endif
	//Begin <JABALL-1500><20150623>Add sun panel function for L5510;xiongdajun
