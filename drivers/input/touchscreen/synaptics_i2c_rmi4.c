    ...
    if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (retval < 0)
            ...
            if (1 ==detected_gestures)
            {
                ...
                #else*/
                gTGesture ='U';
                input_report_key(syna_key_dev,KEY_POWER, 1);
                input_report_key(syna_key_dev,KEY_POWER , 0);
                input_sync(syna_key_dev);
                rmi4_data->sensor_sleep = false;
    			//#endif
            }	
...
static int synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
 ...
 //Begin<REQ><><20150827>add synaptics charger switch;xiongdajun
 #ifdef CONFIG_SYNA_CHARGER_SWITCH
#define CHARGERBIT (1<<5)
int WithCharger = 0;
int old_charger = 0;
extern int usb_plugged_in(void);
extern int g_bat_init_flag;
static int synaptics_charger_switch(struct synaptics_rmi4_data *rmi4_data)
{
        int retval;
        ...
        retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev, "Failed to read device status, error = %d\n", retval);
			return retval;
		}
	...
	dev_err(&rmi4_data->i2c_client->dev,"synaptcis device_ctrl:%d\n",device_ctrl);
...
#endif
//End<REQ><><20150827>add synaptics charger switch;xiongdajun
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
 ...
 //Begin<REQ><><20150827>add synaptics charger switch;xiongdajun
       ...
       if(g_bat_init_flag)
        {
           WithCharger = usb_plugged_in();
           dev_dbg(&rmi4_data->i2c_client->dev,"%s: WithCharger is %d\n",__FUNCTION__,WithCharger);
           if(WithCharger !=old_charger)
                synaptics_charger_switch(rmi4_data);
           old_charger = WithCharger;
        }
#endif
//End<REQ><><20150827>add synaptics charger switch;xiongdajun
